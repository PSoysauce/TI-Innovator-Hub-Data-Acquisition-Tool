readme.txt file  for TI-Innovator-Hub-Data-Acquisition-Tool: (Project name TI-Innovator-Hub-Data-Acquisition-Tool)
Copyright 2021.
...
You can alternatively start an application by running the launcher.exe that is located
in the application's project folder.  This is typically located in the guicomposer folder that is in your Users folder.
e.g:

	c:\Users\<myUserName>\guicomposer\runtime\gcruntime.v<GCRuntime_Major_Version>\<project name>\launcher.exe
...
or by double-clicking on launcher.osx
...
to enable proper execution of the application:
     ...
     sudo apt-get install lib32stdc++6 libc6-i386 libusb-1.0-0-dev:i386 libatomic1
     sudo chmod -R a+rwx ../..
...
In order to get permission to use USB-HID ports(e.g. EVMs that utilize USB2ANY, including TMP sensors), you will need to create a file in /etc/udev/rules.d named
usbhid.rules that contains the following:

    ATTRS(idVendor)=="2047", MODE:="0666"

and restart udev or reboot your PC. 
